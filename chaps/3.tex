...
From time to time, efficient detection methods under limited computational power are very application in various areas, such as driving assistance using embedded sensors. Here, a method pursuing efficiency and real-time detection is proposed.
...
method works in real time, and gives 100\% detection rate and 0\% false alarm rate in one experiment using infrared camera.

As always, detection performance and efficiency are the two important aspects of this method. Challenges come from the need to distinguish noisy objects as shown in Figure~\ref{fig:first} and the real-time requirement. In some experiments, besides the target object a lot of noisy objects also appear. Some of the noisy objects cannot even by distinguished from the target objects by only appearance.  The clutter property of the sensed data makes the detection challenging. This method meets this challenge by making use of both appearance and temporal information of the target objects. There are two main steps in the method. 
...
At first the method is developed for data collected using infrared cameras, then it is extended and carried on data collected by ordinary cameras.

This chapter is organized as follows: section \ref{rw} reviews related work, section 3 proposes pipeline of the method, section 4 gives experimental results, and section 5 concludes.
...
\centering
\subfigure[]{
\includegraphics[width=0.44\textwidth,bb=0 0 640 480]{17Orgimg00039.jpg}
\label{fig:first:a}}
\subfigure[]{
\includegraphics[width=0.44\textwidth,bb=0 0 640 480]{17veriTrjimg00039.jpg}
\label{fig:first:b}}
...
% no \IEEEPARstart
When developing the method, potential applications are within consideration. The method aims to perform detection in real time, and serve as a useful unit in positioning automobiles in tunnel environment.
...
We test this method on detection performance and efficiency.
